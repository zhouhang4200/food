...
//            die;Header("Location: http://www.baidu.com");
            // 获取网站query后缀
            $query = $request->getQueryString();
            myLog('start', ['query' => $query, 'cookie' => cookie('open_id'), 'vue' => $vue]);
//            dd($request->getQueryString());
//            dd(parse_url($request->fullUrl()));
            // 判断当前路由是不是下单选菜页面
            if ($vue == 'order') {
                if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false) { //判断是不是微信
                    myLog('second-wechat', ['query' => $query, 'cookie' => cookie('open_id'), 'vue' => $vue]);
                    if (! cookie('open_id')) { // 不存在openid
                        myLog('three-no-openid', ['query' => $query, 'cookie' => cookie('open_id'), 'vue' => $vue]);
...
//                        }
                        ...
                        myLog('three-have-openid', ['query' => $query, 'cookie' => cookie('open_id'), 'vue' => $vue]);
                        return view('vue');
                    }
                } elseif (strpos($_SERVER['HTTP_USER_AGENT'], 'AlipayClient') !== false) { // 支付宝
                    myLog('second-alipay', ['query' => $query, 'cookie' => cookie('open_id'), 'vue' => $vue]);
                    return view('vue');
                } else { // 都不是
                    myLog('second-none', ['query' => $query, 'cookie' => cookie('open_id'), 'vue' => $vue]);
                    return view('vue');
                }
            }
